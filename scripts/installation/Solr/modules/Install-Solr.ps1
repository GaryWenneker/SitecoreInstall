...
$solrRootWithVersion = "$installFolder\$solrName"
...
downloadAndUnzipIfRequired "Solr" $solrRootWithVersion $solrZip $solrPackage $installFolder
...
if($jreVal -ne $JREPath) {
	Write-Host "JRE Value [$jreVal] is not the same as [$JREPath]"
}
...
if($solrSSL -eq $false)
{
    ...
    if(!(Test-Path -Path "$solrRootWithVersion\bin\solr.in.cmd.old"))
    {
        Write-Host "Rewriting solr config"

        $cfg = Get-Content "$solrRootWithVersion\bin\solr.in.cmd"
        Rename-Item "$solrRootWithVersion\bin\solr.in.cmd" "$solrRootWithVersion\bin\solr.in.cmd.old"
        $newCfg = $newCfg | % { $_ -replace "REM set SOLR_HOST=192.168.1.1", "set SOLR_HOST=$solrHost" }
        $newCfg | Set-Content "$solrRootWithVersion\bin\solr.in.cmd"
    }
}
...
if($solrSSL -eq $true)
{
    ...
    if(!(Test-Path -Path "$solrRootWithVersion\server\etc\solr-ssl.keystore.pfx"))
    {
        Write-Host "Exporting cert for Solr to use"

        $cert = Get-ChildItem Cert:\LocalMachine\Root | where FriendlyName -eq "$solrName"
    
        $certStore = "$solrRootWithVersion\server\etc\solr-ssl.keystore.pfx"
...
if(!($svc))
{
    ...
    &"$installFolder\nssm-$nssmVersion\win64\nssm.exe" install "$solrServiceName" "$solrRootWithVersion\bin\solr.cmd" "start -f" "-p $solrPort"
    $svc = Get-Service "$solrServiceName" -ErrorAction SilentlyContinue
}
